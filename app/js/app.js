...
function progressSummary() {
  ...
  return {
    visas: p.earnedVisas.length,
    days: p.completedDays.length,
...
function taskHref(t) {
  if (t.type === "flashcards") return `#/flashcards?deck=${encodeURIComponent(t.deck)}`;
  if (t.type === "quiz") return `#/quiz/${encodeURIComponent(t.ref)}`;
  if (t.type === "trainer") return "#/trainer";
  if (t.type === "practice") return `#/practice?set=${encodeURIComponent(t.ref)}`;
  if (t.type === "read") return `#/lesson/${encodeURIComponent(t.ref)}`;
  if (t.type === "visa") return "#/visas";
  if (t.type === "recipe") return `#/recipe/${encodeURIComponent(t.ref)}`;
  return "#/lessons";
}

function renderHome() {
  const plan = state.db.workPlan[0];
  const os = state.db.dailyDriverOS[0];
  const counts = {
    flashcards: state.db.flashcards.length,
    questions: state.db.questions.length,
    recipes: state.db.recipes.length,
    variables: state.db.variables.length,
    resources: state.db.resources.length,
  };
  ...
  return `
    <section class="hero">
      ...
      <div class="hero__cta">
        <a class="btn" href="#/plan" data-link>Open 21-day plan</a>
        <a class="btn btn--ghost" href="#/recipes" data-link>Copy a Recipe</a>
      </div>
      <div class="stat-strip">
        <div><strong>${counts.recipes}</strong> recipes</div>
        <div><strong>${counts.variables}</strong> variables</div>
        <div><strong>${counts.flashcards}</strong> flashcards</div>
        <div><strong>${days}/21</strong> days · <strong>${visas}</strong> visas · <strong>${quizzes}</strong> quizzes</div>
      </div>
    </section>
    ...
    <section class="panel">
      <p class="tag">Daily driver OS snapshot</p>
      <h2 style="font-family:var(--font-display);margin:.2rem 0 0.8rem">${escapeHtml(os.name)}</h2>
      <div class="grid grid--3">
        ${os.rituals
          .map(
            (r) => `
          <div>
            <h3 style="font-family:var(--font-display);margin:.2rem 0">${escapeHtml(r.name)} · ${r.minutes}m</h3>
            <ul class="checklist">${r.steps.map((s) => `<li>${escapeHtml(s)}</li>`).join("")}</ul>
          </div>`
          )
          .join("")}
      </div>
      <div style="margin-top:1rem;display:flex;gap:.6rem;flex-wrap:wrap">
        <a class="btn btn--small" href="#/variables" data-link>Variables</a>
        <a class="btn btn--small" href="#/resources" data-link>Docs / videos / audio</a>
        <a class="btn btn--small" href="#/trainer" data-link>Mode Gym</a>
      </div>
    </section>
  `;
}
...
function renderPlan() {
  ...
  return `
    ...
    <h1 class="page-title">21 days to Cursor daily-driver</h1>
    <p class="lede">Week 1 installs habits and safety. Week 2 builds personal + web Recipes. Week 3 adds skills, MCP, Cloud Agents, and your Personal OS README.</p>
        ...
        .map((d) => {
          ...
          <div class="day-row">
            ...
            <div>
              ...
              <div style="margin-top:.35rem;color:rgba(26,36,51,.72)">${escapeHtml(d.checkpoint)}</div>
            </div>
    ...
    <div class="grid grid--2">
      ${days
        ...
        <article class="panel" id="day-${d.day}">
          ...
          <ul class="checklist">
            ${d.tasks
              .map((t) => `<li><a href="${taskHref(t)}" data-link>${escapeHtml(t.label)}</a></li>`)
              .join("")}
          </ul>
...
function renderLessons() {
  return `
    ...
    <h1 class="page-title">Spoon-fed deep dives</h1>
    <p class="lede">Twenty-one modules in plain English. Read the day's lesson, then drill cards and run a Recipe.</p>
    <div class="panel panel--solid">
      ${state.db.modules
        .map(
          (m) => `
        <div class="list-row">
          ...
          <div>
            ...
            <div style="color:rgba(26,36,51,.7)">${escapeHtml(m.subtitle)}</div>
          </div>
...
function renderVariables() {
  return `
    <p class="tag">Variables</p>
    <h1 class="page-title">Fill-in-the-blank power</h1>
    <p class="lede">Every Recipe uses these slots. Copy a token, replace it with your real details, and suddenly prompts become reusable assets.</p>
    <div class="panel panel--solid">
      ${state.db.variables
        .map(
          (v) => `
        <div class="list-row" style="grid-template-columns:1fr">
          <div>
            <p class="tag">${v.requiredOften ? "Use often" : "Optional"} · ${escapeHtml(v.key)}</p>
            <strong><span class="token">${escapeHtml(v.token)}</span></strong>
            <div style="margin-top:.35rem">${escapeHtml(v.meaning)}</div>
            <div class="muted" style="margin-top:.25rem">Example: ${escapeHtml(v.example)}</div>
          </div>
        </div>`
        )
        .join("")}
    ...
    <section class="panel">
      <p class="tag">Mnemonics</p>
      <div class="grid grid--2">
        ${state.db.mnemonics
          .map((m) => `<div><p class="tag">${escapeHtml(m.title)}</p><p>${escapeHtml(m.body)}</p></div>`)
          .join("")}
      </div>
    </section>
  `;
...
function renderRecipes(params) {
  const cats = [...new Set(state.db.recipes.map((r) => r.category))];
  const cat = params.cat || "all";
  const items = state.db.recipes.filter((r) => cat === "all" || r.category === cat);
  return `
    ...
    <div class="panel panel--solid">
      ${items
        .map(
          (r) => `
        <div class="list-row">
          <div class="day-num">${String(r.id).padStart(2, "0")}</div>
          <div>
            <p class="tag">${escapeHtml(r.category)} · ${escapeHtml(r.level)}</p>
            <strong>${escapeHtml(r.title)}</strong>
            <div style="color:rgba(26,36,51,.7)">${escapeHtml(r.whenToUse)}</div>
          </div>
          <a class="btn btn--small" href="#/recipe/${r.slug}" data-link>Open</a>
        </div>`
        )
        .join("")}
    </div>
...
function renderRecipe(slug) {
  const r = state.db.recipes.find((x) => x.slug === slug);
  if (!r) return `<p class="lede">Recipe not found.</p>`;
  return `
    <p class="tag"><a href="#/recipes" data-link>Recipes</a> · ${escapeHtml(r.category)}</p>
    <h1 class="page-title">${escapeHtml(r.title)}</h1>
    <p class="lede">${escapeHtml(r.whenToUse)}</p>
    <article class="panel panel--solid">
      <p class="tag">Variables used</p>
      <p>${r.variablesUsed.map((v) => `<span class="token">{{${escapeHtml(v)}}}</span>`).join("")}</p>
      <p class="tag">Prompt</p>
      <pre class="prompt-box" id="recipePrompt">${escapeHtml(r.prompt)}</pre>
      <div style="display:flex;gap:.6rem;flex-wrap:wrap;margin:1rem 0">
        <button class="btn" id="copyRecipe">Copy prompt</button>
        <a class="btn btn--ghost" href="#/variables" data-link>Variables help</a>
      </div>
      <p><strong>Done when:</strong> ${escapeHtml(r.doneWhen)}</p>
      <p><strong>Safety:</strong> ${escapeHtml(r.safetyNote)}</p>
    </article>
  `;
}

function initRecipe(slug) {
  const r = state.db.recipes.find((x) => x.slug === slug);
  $("#copyRecipe")?.addEventListener("click", async () => {
    try {
      await navigator.clipboard.writeText(r.prompt);
      if (!state.progress.recipesCopied.includes(r.slug)) {
        state.progress.recipesCopied.push(r.slug);
        saveProgress();
      }
      toast("Recipe copied — paste into Cursor");
    } catch {
      toast("Select the prompt box and copy manually");
    }
  });
}

function renderResources() {
  const kinds = [...new Set(state.db.resources.map((r) => r.kind))];
  return `
    <p class="tag">Resources</p>
    <h1 class="page-title">Docs, videos & listen-mode</h1>
    <p class="lede">Explore Cursor to the fullest — official first, then community video searches. Use browser read-aloud on long docs for audio learning.</p>
    ${kinds
      .map((kind) => {
        const items = state.db.resources.filter((r) => r.kind === kind);
        return `
        <section class="panel panel--solid">
          <h2 style="font-family:var(--font-display);margin-top:0;text-transform:capitalize">${escapeHtml(kind)}</h2>
          ${items
            .map(
              (r) => `
            <div class="list-row" style="grid-template-columns:1fr auto">
              <div>
                <p class="tag">Week ${r.week}</p>
                <strong><a href="${escapeHtml(r.url)}" target="_blank" rel="noopener">${escapeHtml(r.title)}</a></strong>
                <div style="color:rgba(26,36,51,.7)">${escapeHtml(r.why)}</div>
              </div>
              <a class="btn btn--small" href="${escapeHtml(r.url)}" target="_blank" rel="noopener">Open</a>
            </div>`
            )
            .join("")}
        </section>`;
...
function renderFlashcards(params) {
  ...
  return `
    ...
    <p class="lede">${cards.length} cards. Flip, then mark Known only when you could teach it to a friend.</p>
    <div class="panel" style="margin-bottom:1rem">
      <label for="deckSelect"><strong>Deck</strong></label>
      <select id="deckSelect" class="search" style="margin-top:.4rem">
        ${decks
          .map((d) => `<option value="${d.id}" ${d.id === deckId ? "selected" : ""}>${escapeHtml(d.title)}</option>`)
          .join("")}
      </select>
    </div>
    <div class="panel panel--solid" id="flashRoot">
      ...
      <div class="flash-actions">
        ...
        <button class="btn btn--small" id="flashKnow" style="background:var(--teal);color:#fff">Known</button>
...
function initFlashcards(params) {
  ...
  cards = [...cards].sort((a, b) => (state.progress.flashSeen[a.id] || 0) - (state.progress.flashSeen[b.id] || 0));
  ...
  function paint() {
    ...
    meta.textContent = "Progress is remembered in this browser.";
  }
...
function renderQuizzes() {
  return `
    ...
    <p class="lede">Pass score is usually 80% (85% on Capstone). Passing stamps the matching Mastery Visa.</p>
    <div class="panel panel--solid">
      ${state.db.quizzes
        .map((q) => {
          ...
          <div class="list-row">
            ...
            <div>
              ...
              <div style="color:rgba(26,36,51,.7)">Best: ${scoreLabel}</div>
            </div>
...
function renderQuiz(id) {
  ...
  return `
    ...
    <div class="panel panel--solid" id="quizRoot">
      <div id="quizBody"></div>
    </div>
  `;
}
...
function renderTrainer() {
  return `
    <p class="tag">Mode Gym</p>
    <h1 class="page-title">Pick the gear</h1>
    <p class="lede">Train Ask / Plan / Agent / Debug reflexes. Best streak saved: ${state.progress.trainerBest}. Hit a streak of 15 for a confidence boost.</p>
    <div class="panel panel--solid" id="trainerRoot">
      ...
      <div class="choice-row choice-row--4" id="trainerChoices"></div>
...
function initTrainer() {
  const prompts = state.db.trainerPrompts.filter((p) => p.kind === "mode");
  const pool = [...prompts];
  let i = 0;
  let score = 0;
  let streak = 0;
  ...
  function show() {
    ...
    const box = $("#trainerChoices");
    box.innerHTML = (p.choices || ["ask", "plan", "agent", "debug"])
      .map((c) => `<button class="btn" data-choice="${escapeHtml(c)}">${escapeHtml(c)}</button>`)
      .join("");
  ...
  function onPick(choice) {
    const p = pool[i % pool.length];
    answered += 1;
    const accepted = p.acceptAny || [p.answer];
    ...
    if (ok) {
      ...
      if (streak >= 15) maybeEarnVisa("mode-picker", "Mode Gym streak");
    } else {
      streak = 0;
      $("#trainerFeedback").textContent = `Not quite. Answer: ${p.answer}. ${p.explain}`;
    }
    i += 1;
    if (i % pool.length === 0) shuffle();
    setTimeout(show, 700);
...
function renderVisas() {
  ...
  return `
    ...
    <p class="lede">Competence stamps — clear the linked quiz and practices. Collect them across the 21-day plan.</p>
    <div class="visa-wall">
...
function renderPractice(params) {
  ...
  return `
    ...
    <h1 class="page-title">Do the reps</h1>
    <p class="lede">Attempt on paper or aloud before revealing. ${items.length} items in this set.</p>
    <div class="panel">
      <label for="setSelect"><strong>Practice set</strong></label>
      <select id="setSelect" class="search" style="margin-top:.4rem">
        ${sets.map((s) => `<option value="${s}" ${s === setId ? "selected" : ""}>${s}</option>`).join("")}
      </select>
    </div>
    <div class="grid">
      ${items
...
function renderReference() {
  return `
    ...
    <p class="lede">Keep these open while briefing Cursor. Search the glossary when a word feels fuzzy.</p>
    <div class="grid grid--2">
    ...
    list.innerHTML = rows
      ...
      <div class="list-row" style="grid-template-columns:1fr">
        <div>
          ...
          <div style="color:rgba(26,36,51,.75)">${escapeHtml(t.definition)}</div>
        </div>
      </div>`
      )
      .join("");
...
async function render() {
  ...
  if (path === "/" || path === "") {
    ...
    html = renderRecipes(params);
    after = () => {
      $("#catSelect")?.addEventListener("change", (e) => {
        navigate(`/recipes?cat=${encodeURIComponent(e.target.value)}`);
      });
    };
  } else if (path.startsWith("/recipe/")) {
    const slug = path.replace("/recipe/", "");
    html = renderRecipe(slug);
    after = () => initRecipe(slug);
